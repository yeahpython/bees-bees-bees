...
# a list of descriptors or hashtags to say what's happening during the time
segdescriptors = []
stickylabels = []
...
def clear_tests():
    ...
    global lines
    global tiles
    global backgroundmarkers
    global backgroundpointmarkers
    global segs
    global segdescriptors
    global stickylabels
    sides = []
    vectors = [(0, 0), (0, 0)]
    indicators = []
...
def summarizetimings(segs, segdescriptors):
    if settings[SHOW_TEXT] and not frames[0] % 30:
        ...
        # print int(w[i] * 10000000), w[i] / total * 100, "%",
        # segdescriptors[i]
        print "\n\n\n"
        ...
        for i, keys in enumerate(segdescriptors):
            if segs[i] >= timeThresholds[curr]:
                for key in keys:
                    ...
                    if index >= n:
                        index = n - 1
                    stardict[key] = stardict[key][
                        :index] + "*" + stardict[key][index + 1:]
                curr += 1
    ...
    for p1, p2 in backgroundmarkers:
        ...
        for x in range(-1, 2, 1):
            for y in range(-1, 2, 1):
                p_1 = p1
                p_2 = p2[0] + x * graphics.world_w, p2[1] + \
                    y * graphics.world_h
                w.append((p_1, p_2))
        # print w
