...
class Camera(object):
    ...
    def __init__(self, player, world, screen, room):
        ...
        self.recs = [
            [pygame.Rect((0, 0), graphics.disp_size) for x in range(3)] for y in range(3)]
        self.sight = []
        self.v_bounded = 1
        self.h_bounded = 1
        self.vxy = matrix([0., 0.])
    ...
    def follow_player(self, dt):
        if 1:
            ...
            # effectively we're checking if we're in the curcle and not too low
            # in the circle
            # or playerfromtracker[0,1] > vertalert:
            if distancefromtracker > trackerradius:
                self.tracker = self.p.xy * 1 + 0.9 * trackerradius * \
                    playerfromtracker / distancefromtracker
