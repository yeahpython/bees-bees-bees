...
class Brain(object):
    ...
    def randomizenodes(self):
        self.nodes = [matrix(np.random.random((1, layer_size)) - 0.5)
    ...
    def randomizeconnections(self):
        ...
        for first, second in zip(self._layer_sizes, self._layer_sizes[1:]):
            self._all_edges.append(
                matrix(10 * np.random.random((first + include_cycles * second, second)) - 5))
            ...
            for r in range(s[0]):
                for c in range(s[1]):
                    if random.random() < settings[MUTATION_CHANCES]:
                        layer[
                            r, c] *= (random.random() * 2 - 1) * settings[SCALING_MUTATION] + 1
                        layer[
                            r, c] += (random.random() * 2 - 1) * settings[ADDITIVE_MUTATION_RATE]
    ...
    def drawfromlocationtable(self, locations, sf, scalefactor=1.0, translation=matrix([0.0, 0.0, 0.0]), detailed=1, rotation=0, shownegatives=1, showlines=1, time=-1):
        ...
        if detailed:
            minconnection, maxconnection = get_connection_strength_range(
                self._all_edges)
            ...
            for i, layer in enumerate(self._all_edges):
                ...
                for j1 in range(s[0]):
                    ...
                    if j1 < self.nodes[i].shape[1]:
                        fir = array(
                            locations[(i, j1)] / scalefactor - translation)[0]
                    elif include_cycles and j1 < self.nodes[i].shape[1] + self.nodes[i + 1].shape[1]:
                        fir = array(
                            locations[(i + 1, j1 - self.nodes[i].shape[1])] / scalefactor - translation)[0]
                    else:
                        break
                    for j2 in range(s[1]):
                        ...
                        if selected and ((i, j1) != selected) and ((i + 1, j2) != selected):
                            continue
                        sec = array(
                            locations[(i + 1, j2)] / scalefactor - translation)[0] * 1
                        ...
                        for index in range(pieces):
                            ...
                            if 0 and index in lightup:
                                ...
                                pygame.draw.line(
                                    sf, [m, m, 0], one2D, two2D, int(width))
                            else:
                                pygame.draw.line(
                                    sf, color, one2D, two2D, int(width))
    ...
    def computevelocities(self, velocities, locations):
        ...
        for n1, l1 in locations.iteritems():
            ...
            for n2, l2 in locations.iteritems():
                ...
                # now applying criterion based on which layer / position the
                # nodes is in
                '''i1, ji = n1[0],  n1[1]
				i2, j2 = n2[0], n2[1]
...
def all_nodes_stationary(velocities, scalefactor):
    # return all(linalg.norm(v) < basicallynotmoving * scalefactor for k,v in
    # velocities.iteritems())
...
def automaticscale(locations):
    ...
    for k, v in locations.iteritems():
        p = array(v)[0]
        dist = linalg.norm(
            v - matrix([graphics.screen_w, graphics.screen_h, 0]))
        maximum = max(dist, maximum)
    return maximum / 2000


def get2DPoint(Point3D, theta):
    rotation = matrix([[math.cos(theta), 0, -math.sin(theta)],
