...
class Bee(physical.Physical):
    ...
    def __init__(self, room, prebrain=None, color=[128, 128, 128],
                 radius=5, health=1, eyepoints=None):
        super(Bee, self).__init__(room)
        if prebrain is not None:
            self.brain = prebrain
        else:
        ...
        self.sharedeyes = [(1, 0), (1, 1), (0, 1), (-1, 0),
                           (1, -1), (-1, 1), (-1, -1), (0, -1)]
        self.total_age = 0
        ...
        if not eyepoints:
            ...
            self.eyepoints = [((70 + 3 * i) * math.cos(3.141592 * 0.13 * i),
                               (70 + 3 * i) * math.sin(3.141592 * 0.13 * i))
                              for i in range(36)]
    ...
    def update_eyes(self):
        ...
        self.eyedrawingoffset = min(
            x for x, y in points) - 5, min(y for x, y in points) - 5
        mx, my = self.eyedrawingoffset

        self.eyemaxes = max(x for x, y in points) + \
            5, max(y for x, y in points) + 5
        Mx, My = self.eyemaxes
        ...
        for i, (x, y) in enumerate(points):
            ...
            pygame.draw.line(
                self.decoration, [255, 255, 255], (x - mx, y - my), (-mx, -my), 1)
            pygame.draw.circle(
                self.decoration, [255, 255, 255], (x - mx, y - my), 4)
    ...
    def sample_path(self, surface):
        ...
        for x in range(steps):
            ...
            if not x:
                ...
                pygame.draw.circle(
                    surface, (0, 0, 0), (int(start[0, 0]), int(start[0, 1])), 30)
                pygame.draw.circle(
                    surface, (255, 255, 255), (int(start[0, 0]), int(start[0, 1])), 30, 1)
                # pygame.draw.circle(surface, (0, 0, 0), (int(start[0, 0]), int(start[0, 1])), w/2)
                pygame.draw.circle(
                    surface, self.color, (int(start[0, 0]), int(start[0, 1])), w * 2 / 5)
            ...
            if abs(start[0, 0] - end[0, 0]) < graphics.world_w / 2:
                if abs(start[0, 1] - end[0, 1]) < graphics.world_h / 2:
                    ...
                    pygame.draw.line(
                        surface, c, (start[0, 0], start[0, 1]), (end[0, 0], end[0, 1]), 1)
            ...
            for x in range(steps):
                ...
                if abs(start[0, 0] - end[0, 0]) < min(graphics.world_w / 2, 200):
                    if abs(start[0, 1] - end[0, 1]) < min(graphics.world_h / 2, 200):
                        ...
                        #w = 1
                        pygame.draw.line(
                            surface, color, (start[0, 0], start[0, 1]), (end[0, 0], end[0, 1]), w)
                        # pygame.draw.line(surface, [0, 0, 0], (start[0, 0], start[0, 1]), (end[0, 0], end[0, 1]), 2)
                if not x:
                    radius = int(
                        10 * (simradius - linalg.norm(offset)) / simradius) + 2
                    radius = steps / 2
                    px = int(
                        (offset + self.player.xy)[0, 0]) % graphics.world_w
                    py = int(
                        (offset + self.player.xy)[0, 1]) % graphics.world_h
    ...
    def eat_player(self):
        ...
        if dist < (self.radius + self.player.forcefield)**2 and dist > 0:
            ...
            self.player.vxy += (dist + 0.01)**-2 * \
                kickdirection * settings[STING_REPULSION]
    ...
    def considergivingbirth(self):
        if len(self.room.bees) < settings[MAXIMUM_BEES] and self.health > 0.9:
            ...
            newbee = Bee(self.room, copy.deepcopy(
                self.brain), self.color, self.radius, eyepoints=self.eyepoints)
            ...
            newbee.treecolor = [randrange(
                max(a - colorvariation, 0), min(a + colorvariation + 1, 255)) for a in newbee.treecolor]
    ...
    def mutate(self):
        ...
        if random.random() > 0.9:
            self.name = random_surname()
        self.color = newcolor
        self.eyes = [(x + random.random() - 0.5, y + random.random() - 0.5)
