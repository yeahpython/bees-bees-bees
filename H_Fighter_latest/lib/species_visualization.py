...
class SpeciesPlot(object):
    ...
    def __init__(self):
        ...
        # print "Visualizing following connections: ", self.in_A, " : ",
        # self.out_A, "; ", self.in_B, " : ", self.out_B
    ...
    def get_representation(self, bee):
        ...
        # bee.player.xy - bee.xy
        disp_to_p = 50 * matrix([math.cos(time.time()), math.sin(time.time())])
        ...
        disps = [disp_to_p[0, 0], disp_to_p[0, 1]]  # not [0.0, 0.0]

        inputs = disps + [1 - distance for distance in infoz] + [bee.health / settings[
            MAX_HEALTH]] + [expit(bee.vxy[0, 0]), expit(bee.vxy[0, 1])]

        bee.brain.compute(inputs, force_no_dropout=True)
        outputs = matrix([0.0, 0.0])
        bee.brain.get_outputs(outputs)
        downup, rightleft = outputs[0, 0], outputs[0, 1]
        ...
        if settings[SPECIES_STYLE] == 1:
            ...
            radius = (len(bee.ancestry) - self.min_ancestry) * \
                1.0 / (self.max_ancestry - self.min_ancestry)
            l = (x ** 2 + y ** 2)**0.5
            if not l:
                l = 1
            x = x / l * radius
            y = y / l * radius
            return (x - math.cos(time.time())) * 0.5, (y - math.sin(time.time())) * 0.5
...
#
        ...
        if settings[SPECIES_STYLE] == 4:
            pygame.draw.circle(
                self.plot, [255, 255, 255], (self.w / 2, self.h / 2), self.w / 2, 1)
        ...
        for b in bees:
            if b.dead == 2:
                continue
            thickness = ((len(b.ancestry) - self.min_ancestry)
                         * maxrad) / (self.max_ancestry - self.min_ancestry)
            ...
            if settings[SPECIES_STYLE] != 3:
                if b.dead:
                    pygame.draw.circle(
                        self.plot, b.treecolor, beepos, thickness + 1, 1)
                    #pygame.draw.circle(self.plot, [255,0,0], beepos, 10, 1)
                else:
                    pygame.draw.circle(
                        self.plot, b.treecolor, beepos, thickness)
