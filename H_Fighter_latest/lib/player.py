...
import graphics
import math
from numpy import linalg, matrix, nditer
import test
import physical
import time
import bullet
...
class Player(physical.Physical):
    ...
    def update(self, dt, key_states, key_presses):
        ...
        myleft, myright, mytop, mybottom = int(
            myleft), int(myright), int(mytop), int(mybottom)
    ...
    '''
        ...
        if self.feetSupported:
            if key_states[pygame.K_LEFT]:
                ...
                if self.normals:
                    # Left # used to be
                    self.vxy += groundacc * \
                        self.normals[0] * matrix([[0, -1], [1, 0]]) * dt
                else:
                ...
                if self.normals:
                    self.vxy += groundacc * \
                        self.normals[0] * \
                        matrix([[0, 1], [-1, 0]]) * dt  # Right
                else:
                    self.vxy += groundacc * matrix([1, 0]) * dt
            elif not self.beepushes or key_states[pygame.K_s]:
                self.vxy *= 0.8
        else:
            self.vxy -= airresist * self.vxy * dt * \
                linalg.norm(self.vxy)  # Air resistance
            if key_states[pygame.K_LEFT]:
                self.vxy -= airacc * dt  # Left
            elif key_states[pygame.K_RIGHT]:
                self.vxy += airacc * dt  # Right
        ...
        assert all(
            val not in [float('infinity'), float('-infinity')] for val in nditer(self.xy))
        ...
        if self.room.tiles[c][r].name == "wall":
            if (self.lastnonwall == 0):
                ...
                self.vxy *= 0.5
        elif self.room.tiles[c][r].name == "bounce":
            self.vxy[0, 1] += -0.3 * dt
        elif self.room.tiles[c][r].name == "end":
            self.vxy[0, 1] += -0.3 * dt
        else:
    ...
    def draw(self, surface):
        ...
        if self.losinghealth:
            ...
            self.displaycolor[0] = 0.9 * \
                self.displaycolor[0] + 0.1 * self.color[0]
            self.displaycolor[1] = 0.9 * \
                self.displaycolor[1] + 0.1 * self.color[1]
            self.displaycolor[2] = 0.9 * \
                self.displaycolor[2] + 0.1 * self.color[2]
        ...
        for x, y in utils.body_copies(self.xy, self.radius):
            ...
            '''right leg'''
            rightfootx = px + 0.4 * self.radius - \
                3 * self.vxy[0, 0] * self.radius
            rightfooty = py + int(self.radius) + 0.1 * self.radius
            rightfootx = px + 0.4 * self.radius * \
                math.cos(self.walkingspeed * pi * self.xy[0, 0])
            if abs(self.vxy[0, 0]) > 0.01:
                rightfooty = py + int(self.radius)
                step = 0.1 * self.radius * \
                    math.sin(self.walkingspeed * pi * self.xy[0, 0])
                if step < 0:
            ...
            rightpoint, leftpoint = [
                int(x) for x in rightpoint], [int(x) for x in leftpoint]
            ...
            righthandx = shoulderx + 0.4 * self.radius * \
                math.cos(
                    self.walkingspeed * pi * self.xy[0, 0]) * (0.2 + min(abs(self.vxy[0, 0]), 0.1))**0.5 * 2
            ...
            lefthandx = shoulderx - 0.4 * self.radius * \
                math.cos(
                    self.walkingspeed * pi * self.xy[0, 0]) * (0.2 + min(abs(self.vxy[0, 0]), 0.1))**0.5 * 2
            ...
            try:
                ...
                pygame.draw.circle(
                    surface, color, head, int(self.radius * 0.2), 0)
