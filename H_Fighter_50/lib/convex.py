...
class Convex(object):
	def __init__(self, pointlist, offset):
		...
		self.use_pointwise = {p:0 for p in self.points}

	def repel(self, other = None, circle = None, radius = -1, corners = 0):
		'''returns the minimum displacement, if any, required to push the other convex out'''
		
		'''set the normals to consider'''
		ns_to_consider = []
		my_ns = [k for k,v in self.shadowDict.iteritems()]
		other_ns = []
		...
		if circle:
			...
			other_ns = [k for k,v in other.shadowDict.iteritems()]
